...
DEFAULT_OUTPUT = "outputs/debug_smoke/qwen3_alfworld_state_longhist_smoke.json"


def _with_fillers(task: Dict[str, Any], filler_prefix: str, filler_count: int) -> Dict[str, Any]:
    task = dict(task)
...
def build_cases(filler_count: int) -> List[Dict[str, Any]]:
    return [
        _with_fillers(
            {
                "case_name": "lookat_state_ready_longhist",
            ...
            filler_prefix="lookat_state_ready_longhist",
            filler_count=filler_count,
        ),
        _with_fillers(
            {
                "case_name": "heat_post_state_longhist",
            ...
            filler_prefix="heat_post_state_longhist",
            filler_count=filler_count,
        ),
        _with_fillers(
            {
                "case_name": "cool_post_state_longhist",
            ...
            filler_prefix="cool_post_state_longhist",
            filler_count=filler_count,
        ),
        _with_fillers(
            {
                "case_name": "clean_post_state_longhist",
            ...
            filler_prefix="clean_post_state_longhist",
            filler_count=filler_count,
        ),
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Smoke-test long-history state families for Qwen3 OCR baseline vs idea.")
    parser.add_argument("--model-path", default=DEFAULT_MODEL_PATH)
    parser.add_argument("--output", default=DEFAULT_OUTPUT)
    parser.add_argument("--device", default="cuda:0")
    parser.add_argument("--max-new-tokens", type=int, default=160)
    parser.add_argument("--max-prompt-length", type=int, default=2048)
    parser.add_argument("--filler-count", type=int, default=12)
    ...
    with temporary_env(env_overrides):
        for task in build_cases(args.filler_count):
            for variant in ("baseline", "idea"):
