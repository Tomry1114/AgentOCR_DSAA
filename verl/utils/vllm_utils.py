...
OPTIONAL_VLLM_IMPORT_ERRORS = (ImportError, AttributeError, RuntimeError)
...
except OPTIONAL_VLLM_IMPORT_ERRORS:
    pass
    ...
    def hijack():
        def hijack__load_adapter(self, lora_request: TensorLoRARequest) -> LoRAModel:
            ...
            try:
                ...
                lora_extra_vocab_size = getattr(self.lora_config,
                                               "lora_extra_vocab_size", 0)
                embedding_modules = getattr(self, "embedding_modules", None)
                embedding_padding_modules = getattr(self,
                ...
                if isinstance(lora_request, TensorLoRARequest):
                    ...
                    tensor_sig = inspect.signature(
                        self._lora_model_cls.from_lora_tensors)
                    if "embeddings" in tensor_sig.parameters:
                        tensor_kwargs["embeddings"] = None
                    if "target_embedding_padding" in tensor_sig.parameters:
                        tensor_kwargs["target_embedding_padding"] = (
                            self.vocab_size + lora_extra_vocab_size)
                    if "embedding_modules" in tensor_sig.parameters:
                        tensor_kwargs["embedding_modules"] = embedding_modules
                    if "embedding_padding_modules" in tensor_sig.parameters:
                        tensor_kwargs["embedding_padding_modules"] = (
                            embedding_padding_modules)
                    if "model_vocab_size" in tensor_sig.parameters:
                        tensor_kwargs["model_vocab_size"] = self.vocab_size
                    ...
                    ckpt_sig = inspect.signature(
                        self._lora_model_cls.from_local_checkpoint)
                    if "target_embedding_padding" in ckpt_sig.parameters:
                        ckpt_kwargs["target_embedding_padding"] = (
                            self.vocab_size + lora_extra_vocab_size)
                    if "embedding_modules" in ckpt_sig.parameters:
                        ckpt_kwargs["embedding_modules"] = embedding_modules
                    if "embedding_padding_modules" in ckpt_sig.parameters:
                        ckpt_kwargs["embedding_padding_modules"] = (
                            embedding_padding_modules)
                    if "model_vocab_size" in ckpt_sig.parameters:
                        ckpt_kwargs["model_vocab_size"] = self.vocab_size
            ...
            actual_extra_vocab_size = getattr(lora, "extra_vocab_size", 0)
            if actual_extra_vocab_size > lora_extra_vocab_size:
                raise ValueError(f"LoRA added vocab size {actual_extra_vocab_size} "
