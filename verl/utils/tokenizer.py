...
def ensure_special_tokens_extended(tokenizer):
    """Backfill tokenizer APIs expected by older vLLM releases."""
    ...
    setattr(tokenizer, "all_special_tokens_extended", extended_tokens)
    return tokenizer


def patch_transformers_tokenizer_compat():
    """Patch tokenizer base classes so newly created tokenizers expose legacy attrs."""
...
def hf_tokenizer(name_or_path, correct_pad_token=True, correct_gemma2=True, **kwargs):
    ...
    ensure_special_tokens_extended(tokenizer)
    return tokenizer
