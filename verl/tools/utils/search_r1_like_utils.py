...
def _post_search_request(retrieval_service_url: str, payload: Dict[str, Any], timeout: int):
...
def _fallback_single_query_api(retrieval_service_url: str, query_list: List[str], topk: int, timeout: int):
    combined_results = []
    for query in query_list:
        ...
        if not isinstance(response_results, list):
            raise ValueError(f"Unexpected single-query search response format: {response_json}")
        ...
        try:
            ...
            if response.status_code in [400, 404, 405, 422] and query_list:
                logger.warning(
                    ...
                    "falling back to single-query API compatibility mode."
                )
                fallback_response = _fallback_single_query_api(
                    retrieval_service_url=retrieval_service_url,
