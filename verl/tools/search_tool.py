...
class SearchTool(BaseTool):
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema):
        ...
        self._next_url_idx = 0

        raw_retrieval_service_url = config.get("retrieval_service_url")
        if isinstance(raw_retrieval_service_url, str):
            self.retrieval_service_urls = [raw_retrieval_service_url]
        elif isinstance(raw_retrieval_service_url, Sequence):
            self.retrieval_service_urls = [str(url) for url in raw_retrieval_service_url if str(url)]
        else:
            self.retrieval_service_urls = []
        assert self.retrieval_service_urls, "Configuration must include 'retrieval_service_url'"
        if any(url == "" for url in self.retrieval_service_urls):
            raise ValueError("retrieval_service_url is not set")

        # Worker and rate limiting configuration
        default_parallelism = max(4, min(8 * len(self.retrieval_service_urls), 32))
        self.num_workers = int(config.get("num_workers", default_parallelism))
        self.rate_limit = int(config.get("rate_limit", default_parallelism))
    ...
    def _pick_retrieval_service_url(self) -> str:
        with self._url_lock:
            url = self.retrieval_service_urls[self._next_url_idx % len(self.retrieval_service_urls)]
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> Tuple[str, float, dict]:
        ...
        try:
            retrieval_service_url = self._pick_retrieval_service_url()
            result_text, metadata = await self.execution_pool.execute.remote(self.execute_search, instance_id, query_list_from_params, retrieval_service_url, self.topk, timeout)
