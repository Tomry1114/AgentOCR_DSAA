...
def _agentocr_debug_flag_enabled(env_name: str, default: str = "0") -> bool:
    return os.environ.get(env_name, default).strip().lower() in {"1", "true", "yes", "on"}


def _agentocr_non_tensor_value(values, idx: int):
...
def _agentocr_extract_question(env_kwargs) -> str:
...
def _agentocr_truncate_debug_text(text: str, max_chars: int = 96) -> str:
...
def _agentocr_log_batch_signature(tag: str, step: int, batch: DataProto) -> None:
    if not _agentocr_debug_flag_enabled("AGENTOCR_LOG_BATCH_SIGNATURE"):
        return

    non_tensor_batch = getattr(batch, "non_tensor_batch", {}) or {}
    env_kwargs_batch = non_tensor_batch.get("env_kwargs")
    data_sources = non_tensor_batch.get("data_source")
    dataset_indices = non_tensor_batch.get("index")

    candidate_lengths = [
        len(values)
        for values in (env_kwargs_batch, data_sources, dataset_indices)
        if values is not None
    ]
    if not candidate_lengths:
        return

    batch_size = max(candidate_lengths)
    preview_limit = max(1, int(os.environ.get("AGENTOCR_LOG_BATCH_SIGNATURE_LIMIT", "8")))

    full_signature = []
    preview_signature = []
    for sample_idx in range(batch_size):
        env_kwargs = _agentocr_non_tensor_value(env_kwargs_batch, sample_idx)
        entry = {
            "sample_idx": sample_idx,
            "data_source": str(_agentocr_non_tensor_value(data_sources, sample_idx) or ""),
            "dataset_index": _agentocr_non_tensor_value(dataset_indices, sample_idx),
            "question": _agentocr_extract_question(env_kwargs),
        }
        full_signature.append(entry)
        if sample_idx < preview_limit:
            preview_entry = dict(entry)
            preview_entry["question"] = _agentocr_truncate_debug_text(preview_entry["question"])
            preview_signature.append(preview_entry)

    fingerprint_payload = json.dumps(full_signature, ensure_ascii=False, sort_keys=True)
    fingerprint = hashlib.sha1(fingerprint_payload.encode("utf-8")).hexdigest()[:12]
    print(
        "[AGENTOCR_BATCH_SIGNATURE] "
        f"tag={tag} step={step} size={batch_size} fingerprint={fingerprint} "
        f"preview={json.dumps(preview_signature, ensure_ascii=False)}",
        flush=True,
    )
...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # We only do validation on rule-based reward models. Be defensive
            # here because some datasets may not carry reward_model metadata.
            reward_model_info = test_batch[0].non_tensor_batch.get("reward_model", {})
            reward_model_style = reward_model_info.get("style") if isinstance(reward_model_info, dict) else None
            if self.config.reward_model.enable and reward_model_style == "model":
            ...
            non_tensor_batch_keys_to_pop = ["raw_prompt_ids", "data_source", "index"]
            ...
            _agentocr_log_batch_signature("val", step=self.global_steps, batch=test_gen_batch)
            ...
            for key, value in test_batch.non_tensor_batch.items():
                if not key.startswith('trust_policy/'):
                    continue
                if key not in trust_policy_metric_values:
                    trust_policy_metric_values[key] = []
                trust_policy_metric_values[key].append(np.asarray(value, dtype=np.float32))
            for k in test_batch.non_tensor_batch.keys():
                if not is_task_success_metric(k):
                    continue
                if k not in success_rate_dict:
                    success_rate_dict[k] = []
                success_rate_dict[k].append(test_batch.non_tensor_batch[k][0])
                # Task success metrics are batch-level constants after
                # rollout_loop aggregates them with np.mean.
        ...
        for key, chunks in trust_policy_metric_values.items():
            if not chunks:
                continue
            values = np.concatenate(chunks, axis=0)
            if values.size == 0:
                continue
            metric_key = key if key.endswith(('_fraction', '_rate', '_mean')) else f'{key}_rate'
            metric_dict[f'val/{metric_key}'] = float(np.mean(values))
    ...
    def init_workers(self):
        ...
        """
        debug_boot = os.environ.get("AGENTOCR_BOOT_DEBUG") == "1"

        def debug(message: str) -> None:
            if debug_boot:
                print(f"[AGENTOCR_INIT_WORKERS] {message}", flush=True)

        debug("create_resource_pool start")
        self.resource_pool_manager.create_resource_pool()
        debug("create_resource_pool done")
        ...
        for resource_pool, class_dict in self.resource_pool_to_cls.items():
            debug(f"worker group setup start roles={list(class_dict.keys())}")
            worker_dict_cls = create_colocated_worker_cls(class_dict=class_dict)
            debug(f"worker group class created roles={list(class_dict.keys())}")
            wg_dict = self.ray_worker_group_cls(resource_pool=resource_pool, ray_cls_with_init=worker_dict_cls, device_name=self.device_name, **wg_kwargs)
            debug(f"worker group instance created roles={list(class_dict.keys())}")
            spawn_wg = wg_dict.spawn(prefix_set=class_dict.keys())
            debug(f"worker group spawned roles={list(spawn_wg.keys())}")
            all_wg.update(spawn_wg)

        if self.use_critic:
            self.critic_wg = all_wg["critic"]
            debug("critic_wg.init_model start")
            self.critic_wg.init_model()
            debug("critic_wg.init_model done")

        if self.use_reference_policy and not self.ref_in_actor:
            self.ref_policy_wg = all_wg["ref"]
            debug("ref_policy_wg.init_model start")
            self.ref_policy_wg.init_model()
            debug("ref_policy_wg.init_model done")

        if self.use_rm:
            self.rm_wg = all_wg["rm"]
            debug("rm_wg.init_model start")
            self.rm_wg.init_model()
            debug("rm_wg.init_model done")

        # we should create rollout at the end so that vllm can have a better estimation of kv cache memory
        self.actor_rollout_wg = all_wg["actor_rollout"]
        debug("actor_rollout_wg.init_model start")
        self.actor_rollout_wg.init_model()
        debug("actor_rollout_wg.init_model done")
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get("val_before_train", True):
            ...
            if val_metrics:
                ...
                print("Initial validation returned no metrics; skipping validation log.")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                _agentocr_log_batch_signature("train", step=self.global_steps, batch=gen_batch)
