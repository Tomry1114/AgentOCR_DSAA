...
def _to_metric_scalar(value: Any) -> float:
    if isinstance(value, torch.Tensor):
        return float(value.reshape(-1)[0].item())
    if isinstance(value, np.ndarray):
        if value.size == 0:
            return 0.0
        return _to_metric_scalar(value.reshape(-1)[0])
    if isinstance(value, (list, tuple)):
        if not value:
            return 0.0
        return _to_metric_scalar(value[0])
    if hasattr(value, "item"):
        return float(value.item())
    return float(value)


def is_task_success_metric(metric_name: str) -> bool:
    # Task-level outcome metrics propagated through rollout_loop are flat names
    # such as "success_rate", "<data_source>_success_rate", or
    # "<task_family>_success_rate". WebShop also exposes a dense purchase score
    # via `task_score`, which should be logged alongside strict success.
    if "/" in metric_name:
        return False
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    metrics = {
        ...
        **({f"episode/{k}": _to_metric_scalar(v) for k, v in batch.non_tensor_batch.items() if is_task_success_metric(k)}),
    }
    ...
    for key, value in batch.non_tensor_batch.items():
        ...
        metrics[metric_key] = float(array.mean())

    for key, value in batch.non_tensor_batch.items():
        if not key.startswith("memory_credit/"):
            continue
        try:
            array = np.asarray(value, dtype=np.float32)
