...
if _flash_attn_import_error is not None:
    logger.warning("flash_attn import failed in qwen2_vl, falling back to non-flash attention: %s", _flash_attn_import_error)
...
def _sdpa_attention_forward(
    ...
    if sdpa_mask is not None and sdpa_mask.dim() == 2:
        sdpa_mask = sdpa_mask[:, None, None, :].to(dtype=torch.bool)
...
def _custom_flash_attention_forward(
    ...
    **kwargs,
    ...
    if (
        ...
        attn_output = _sdpa_attention_forward(
...
def process_position_ids(position_ids: torch.Tensor) -> torch.Tensor:
    ...
    if position_ids.ndim != 3:
        raise ValueError(f"position_ids should be a 3D tensor, got {position_ids.shape}")

    # Newer HF/VLM call paths may provide position ids as (batch_size, 4, seq_length)
    # while the patched Qwen2.5-VL forward expects (4, batch_size, seq_length).
    if position_ids.size(0) == 4:
        pass
    elif position_ids.size(1) == 4:
        ...
        raise ValueError(
            "position_ids should have shape (4, batch_size, seq_length) or (batch_size, 4, seq_length), "
