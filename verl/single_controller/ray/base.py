...
class RayWorkerGroup(WorkerGroup):
    ...
    def _init_with_resource_pool(self, resource_pool, ray_cls_with_init, bin_pack, detached):
        ...
        def debug(message: str) -> None:
            if debug_boot:
                print(f"[AGENTOCR_RAY_WG] {message}", flush=True)
        ...
        debug(f"get_placement_groups start strategy={strategy} use_gpu={use_gpu} pool_store={resource_pool.store} max_colocate_count={resource_pool.max_colocate_count}")
        pgs = resource_pool.get_placement_groups(strategy=strategy, device_name=self.device_name)
        debug(f"get_placement_groups done count={len(pgs)}")
        ...
        for pg_idx, pg in enumerate(sort_placement_group_by_node_ip(pgs)):
            debug(f"placement group loop start pg_idx={pg_idx} bundle_count={pg.bundle_count}")
            ...
            for local_rank in range(local_world_size):
                ...
                for passthrough_key in ("VERL_HF_ATTN_IMPLEMENTATION", "VLLM_ATTENTION_BACKEND"):
                    passthrough_value = os.environ.get(passthrough_key)
                    if passthrough_value is not None:
                        env_vars[passthrough_key] = passthrough_value
                if rank != 0:
                ...
                # create a worker
                debug(f"create worker start rank={rank} pg_idx={pg_idx} local_rank={local_rank} name={name}")
                ...
                debug(f"create worker done rank={rank} name={name}")
                ...
                if rank == 0:
                    ...
                    debug(f"wait register center start actor_name={actor_name}")
                    ...
                    while time.time() - start_time < self._ray_wait_register_center_timeout:
                        if actor_name in list_named_actors():
                            ...
                            debug(f"wait register center found actor_name={actor_name}")
                            break
                    ...
                    debug(f"rank zero info received master_addr={self._master_addr} master_port={self._master_port}")
