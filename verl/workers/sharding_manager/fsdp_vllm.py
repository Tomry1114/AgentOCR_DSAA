...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __enter__(self):
        ...
        def __collect_lora_params()->OrderedDict:
            ...
            work with if isinstance(wrapped_module, PeftModel)
            """
    ...
    def update_params(self, updated_params, peft_config=None):
        ...
        if peft_config:
            if self.base_sync_done:
                ...
                # vLLM currently applies LoRA only to the language model in
                # multimodal architectures. Keep the vision tower weights
                # loaded from the original checkpoint and only synchronize
                # language-side parameters during the initial base sync.
                if hasattr(model, "visual") and hasattr(model, "language_model"):
                    multimodal_skip_prefixes = (
                        "model.visual.",
                        "visual.",
                    )
                    filtered_params = {
                        ...
                        if not k.startswith(multimodal_skip_prefixes)
                    }
                    skipped_count = len(updated_params) - len(filtered_params)
                    if skipped_count > 0:
                        logger.info(
                            "Skip %d multimodal vision params during initial vLLM base sync for LoRA model",
                            skipped_count,
                        )
