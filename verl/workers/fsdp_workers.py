...
def resolve_hf_attn_implementation(default: str = "flash_attention_2") -> str:
    attn_impl = os.getenv("VERL_HF_ATTN_IMPLEMENTATION", default)
    ...
    try:
        ...
        logger.warning("flash_attn unavailable, falling back to sdpa: %s", exc)
        return "sdpa"
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        attn_implementation = resolve_hf_attn_implementation()
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        rollout_module = getattr(self.rollout, "module", None)
        should_reload_rollout_module = self._is_offload_param and isinstance(rollout_module, FSDP)

        if should_reload_rollout_module:
        ...
        if debug_trace:
            try:
                raw_prompt_text = prompts.non_tensor_batch.get("raw_prompt_text")
                mm_batch = prompts.non_tensor_batch.get("multi_modal_data")
                mm_present = mm_batch is not None
                image_counts = []
                if mm_present:
                    for item in mm_batch[: min(4, len(mm_batch))]:
                ...
                print(
                    "[AgentOCR VLLM Debug][fsdp_worker] "
                    f"enter generate_sequences batch={len(prompts)} "
                    f"mm_present={mm_present} image_counts_head={image_counts} "
                    f"has_raw_prompt_text={raw_prompt_text is not None}",
                    flush=True,
                )
                faulthandler.dump_traceback_later(debug_timeout_sec, repeat=True)
                debug_context = "armed"
            except Exception as exc:
                print(f"[AgentOCR VLLM Debug][fsdp_worker] failed to arm traceback dump: {exc}", flush=True)
        try:
            with self.rollout_sharding_manager:
                log_gpu_memory_usage("After entering rollout sharding manager", logger=logger)
                if debug_trace:
                    print("[AgentOCR VLLM Debug][fsdp_worker] entered rollout_sharding_manager", flush=True)

                prompts = self.rollout_sharding_manager.preprocess_data(prompts)
                if debug_trace:
                    print("[AgentOCR VLLM Debug][fsdp_worker] after preprocess_data", flush=True)
                output = self.rollout.generate_sequences(prompts=prompts)
                if debug_trace:
                    print("[AgentOCR VLLM Debug][fsdp_worker] after rollout.generate_sequences", flush=True)

                log_gpu_memory_usage("After rollout generation", logger=logger)

                output = self.rollout_sharding_manager.postprocess_data(output)
                if debug_trace:
                    print("[AgentOCR VLLM Debug][fsdp_worker] after postprocess_data", flush=True)
        finally:
            if debug_context == "armed":
