...
class WebshopWorker:
    ...
    def _get_goal_metadata(self):
        """Expose the active goal so higher-level prompt builders can form better search actions."""
        goal = {}
        try:
            session_id = getattr(self.env, "session", None)
            user_sessions = getattr(getattr(self.env, "server", None), "user_sessions", {})
            if session_id in user_sessions:
                goal = user_sessions[session_id].get("goal", {}) or {}
        except Exception:
            goal = {}
        ...
        attributes = [
            str(item).strip()
            for item in (goal.get("attributes") or [])
            if str(item).strip()
        ]
        instruction_text = str(goal.get("instruction_text", "") or "").strip()
        product_category = str(goal.get("product_category", "") or "").strip()
        target_asin = str(goal.get("asin", "") or "").strip()
        return {
            "goal_query": query,
            "goal_attributes": attributes,
            "goal_instruction_text": instruction_text,
            "goal_product_category": product_category,
            "goal_target_asin": target_asin,
        }
    ...
    def step(self, action):
        ...
        available_actions.update(self._get_goal_metadata())
