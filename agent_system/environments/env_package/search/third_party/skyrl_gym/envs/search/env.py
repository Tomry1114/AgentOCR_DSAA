...
class SearchEnv(BaseTextEnv):
    ...
    def __init__(self, env_config: DictConfig):
        ...
        self.tool_group = SearchToolGroup(
            ...
            max_concurrent_requests=getattr(env_config, "max_concurrent_requests", 8),
        )
    ...
    def step(self, action: str) -> BaseTextEnvStepOutput:
        ...
        reward = self._get_reward(postprocessed_action, done)
        ...
        try:
            query = self._parse_action(postprocessed_action)
            if query[0] and adapt_meta.get("deserves_search_bonus", True):
