...
def _agentocr_boot_debug(message):
...
class AlfworldWorker:
    ...
    def __init__(self, config, seed, base_env):
        _agentocr_boot_debug(f"AlfworldWorker init start seed={seed}")
        self.env = base_env.init_env(batch_size=1)  # Each worker holds only one sub-environment
        self.env.seed(seed)
        _agentocr_boot_debug(f"AlfworldWorker init done seed={seed}")
    ...
    def close(self):
        """Gracefully close the underlying environment before the actor is killed."""
...
class AlfworldEnvs(gym.Env):
    def __init__(self, alf_config_path, seed, env_num, group_n, resources_per_worker, is_train=True, env_kwargs={}):
        ...
        _agentocr_boot_debug(
            f"AlfworldEnvs init start split={'train' if is_train else 'eval'} env_num={env_num} group_n={group_n} resources={resources_per_worker}"
        )
        
        # Initialize Ray if not already initialized
        if not ray.is_initialized():
            _agentocr_boot_debug("AlfworldEnvs ray.init start")
            ray.init()
            _agentocr_boot_debug("AlfworldEnvs ray.init done")
            
        eval_dataset = env_kwargs.get('eval_dataset', 'eval_in_distribution')
        _agentocr_boot_debug("AlfworldEnvs load config start")
        config = load_config_file(alf_config_path)
        env_type = config['env']['type']
        _agentocr_boot_debug(f"AlfworldEnvs base env start env_type={env_type}")
        base_env = get_environment(env_type)(config, train_eval='train' if is_train else eval_dataset)
        _agentocr_boot_debug(f"AlfworldEnvs base env done env_type={env_type}")
        ...
        for i in range(self.num_processes):
            _agentocr_boot_debug(f"AlfworldEnvs actor create start index={i}")
            worker = env_worker.remote(config, seed + (i // self.group_n), base_env)
            self.workers.append(worker)
            _agentocr_boot_debug(f"AlfworldEnvs actor create done index={i}")

        self.prev_admissible_commands = [None for _ in range(self.num_processes)]
        _agentocr_boot_debug(f"AlfworldEnvs init done num_processes={self.num_processes}")
