...
config_file="${QWEN3VL4B_TEXT_4XA800_CONFIG:-$(pwd)/configs/qwen3vl4b_text_4xa800.env}"
...
seed="${SEED}"
experiment_name="${TRAINER_EXPERIMENT_NAME:-agentocr_qwen3vl4b_text_4xa800_s${seed}}"
prepare_local_dir="${PREPARE_LOCAL_DIR:-$HOME/data/verl-agent-q3vl4b-text/}"
skip_data_prepare="${SKIP_DATA_PREPARE:-0}"
prepared_local_root="${prepare_local_dir/#\~/$HOME}"
prepared_text_dir="${prepared_local_root%/}/text"
...
if [ "$skip_data_prepare" = "1" ] || [ "$skip_data_prepare" = "true" ] || [ "$skip_data_prepare" = "True" ]; then
    ...
    echo "Preparing shared text parquet with original ALFWorld split logic: train=${TRAIN_DATA_SIZE} test=$((VAL_DATA_SIZE * 2))"
    ...
    export TRAIN_FILE="${prepared_text_dir}/train.parquet"
    export VAL_FILE="${prepared_text_dir}/test.parquet"
    echo "Using freshly prepared parquet files: train=${TRAIN_FILE} val=${VAL_FILE}"
fi

echo "Launching 4xA800 Qwen3-VL-4B text-only run: seed=${seed} experiment=${experiment_name}"
echo "Config file: ${config_file}"
echo "python_bin=${python_bin}"
echo "cuda_home=${CUDA_HOME:-}"
echo "nvcc=$(command -v nvcc || echo missing)"
echo "actor_lr=${ACTOR_LR}"
echo "prompt_length_cap=${DATA_MAX_PROMPT_LENGTH}"
